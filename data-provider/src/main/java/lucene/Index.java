...
public class Index {
	...
	public void mergeIndex(String[] indexesPaths,String mergePath){
			try{
				Directory[] directories = new Directory[indexesPaths.length];
				for (int i = 0; i <indexesPaths.length ; i++) {
					directories[i] = FSDirectory.open(new File(indexesPaths[i]));
				}

				Directory output=FSDirectory.open(new File(mergePath));//合并到索引3里面
	...
	public void optimize(String indexPath) throws IOException {
		IndexWriter indexWriter=null;
