 ...
 */
public class ZgjybEduCrawler extends BaseEduCrawler {
    ...
    public void crawlUrlList() {
        ...
        for (int i =1; i <366; i++) {
            int pagenum=0;
            url ="http://paper.jyb.cn/zgjyb/html/"+date(i)+"/node_2.htm";
            String str="http://paper.jyb.cn/zgjyb/html/"+date(i)+"/";      //保存为了后续url地址的构造
    ...
    public void getUrlList(HtmlPage page,String linkUrl,String str,int pagenum) throws InterruptedException, IOException{
        ...
        Elements eles = doc.select("div#titleList1 li");
        String url="";
        for (Element ele : eles) {
               Element e=ele.select("a[href]").first();
               url=str+e.attr("href");
               urls.add(url);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ZgjybEduCrawler crawler = new ZgjybEduCrawler();
        crawler.crawlUrlList();
        crawler.crawlPage(urls,"中国教育报");
    }
}
