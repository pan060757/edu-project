 ...
 */
package edu.ecnu.spcrawler.ZgjybEdu;
...
public class ContentExtractor {
    ...
    public NewsPage getNews() {
        ...
        try {
            ...
            news.setContent(getContentByDoc(this.doc));
        } catch (Exception ex) {
        ...
        news.setHtml(this.doc.html());
        return news;
    }


    public NewsPage getNews(Configuration conf, Document doc) {
        Element e = doc.body();
        ...
        newsPage.setSourceId(sourceId);
        newsPage.setPageId(UUID.randomUUID().toString().replaceAll("-", ""));
        if (doc.baseUri() != null) {
            newsPage.setUrl(doc.baseUri());
        }
        try {
            contentElement = getContentElement();
            newsPage.setContent(getContentByDoc(this.doc));
        } catch (Exception ex) {
            LOG.info("news content extraction failed,extraction abort", ex);
            String message = newsPage.getSourceId() + "\t" + newsPage.getPublisher() + "\t" + newsPage.getUrl() + "\t" + ex.getClass().getName() + "\t" + "0";
            //    sc.sendMessage(message);
        }
        try {
            newsPage.setTime(getTime(e));
        } catch (Exception ex) {
            LOG.info("news time extraction failed", ex);
            String message = newsPage.getSourceId() + "\t" + newsPage.getPublisher() + "\t" + newsPage.getUrl() + "\t" + ex.getClass().getName() + "\t" + "0";
            //    sc.sendMessage(message);
            //  newsPage.setTime(null);
        }
        try {
            newsPage.setTitle(getTitle(contentElement));
        } catch (Exception ex) {
            LOG.info("title extraction failed", ex);
            String message = newsPage.getSourceId() + "\t" + newsPage.getPublisher() + "\t" + newsPage.getUrl() + "\t" + ex.getClass().getName() + "\t" + "0";
            //      sc.sendMessage(message);
        }
        newsPage.setHtml(this.doc.html());
        return newsPage;
    }

    protected String getTime(Element contentElement) throws Exception {
        String regex = "([1-2][0-9]{3})[^0-9]([0-1]?[0-9])[^0-9]([0-9]{1,2})[^0-9]([0-2]?[0-9])[^0-9]([0-9]{1,2})[^0-9]?([0-9]{1,2})?";
        ...
        for (int i = 0; i < 6; i++) {
            ...
            } else if (matcher.find() && matcher.group(6) == null) {
    ...
    protected String getDate(Element contentElement) throws Exception {
        //String regex = "([1-2][0-9]{3})[^0-9]([0-1]?[0-9])[^0-9]([0-9]{1,2})[^0-9]?";
        String regex = "([1-2][0-9]{3})[^0-9]([0-1]?[0-9])[^0-9]([0-9]{1,2})[^0-9]?";
        Pattern pattern = Pattern.compile(regex);
        Element current = contentElement;
        for (int i = 0; i < 2; i++) {
    ...
    /*输入URL，获取正文所在Element*/
    public static Element getContentElementByUrl(String url) {
    ...
    public static NewsPage getNewsByDoc(Configuration conf, Document doc) {
        ContentExtractor ce = new ContentExtractor(doc);
        return ce.getNews(conf, doc);
    }

    /*输入Jsoup的Document，获取结构化新闻信息*/
    public static NewsPage getNewsByDoc(Document doc) {
        ContentExtractor ce = new ContentExtractor(doc);
        return ce.getNews();
    }

    /*输入HTML，获取结构化新闻信息*/
    public static NewsPage getNewsByHtml(Configuration conf, String html) throws Exception {
        Document doc = Jsoup.parse(html);
        return getNewsByDoc(conf, doc);
    }

    /*输入HTML和URL，获取结构化新闻信息*/
    public static NewsPage getNewsByHtml(Configuration conf, String html, String url) {
        Document doc = Jsoup.parse(html, url);
        try {
            return getNewsByDoc(conf, doc);
        } catch (Exception e) {
    ...
    /*输入HTML和URL，获取结构化新闻信息*/
    public static NewsPage getNewsByHtml(String html, String url) {
        Document doc = Jsoup.parse(html, url);
        try {
    ...
    /*输入URL，获取结构化新闻信息*/
    public static NewsPage getNewsByUrl(Configuration conf, String url) {
        HttpRequest request = null;
        try {
            request = new HttpRequest(url);
            String html = request.getResponse().getHtmlByCharsetDetect();

            return getNewsByHtml(conf, html, url);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    /*输入URL，获取结构化新闻信息*/
    public static NewsPage getNewsByUrl(String url) {
        HttpRequest request = null;
        try {
            request = new HttpRequest(url);
            String html = request.getResponse().getHtmlByCharsetDetect();
...
//        NewsPage news = ContentExtractor.getNewsByUrl("http://www.huxiu.com/article/121959/1.html");
