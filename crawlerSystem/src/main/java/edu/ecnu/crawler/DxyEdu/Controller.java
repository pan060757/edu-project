package edu.ecnu.crawler.DxyEdu;
...
public class Controller extends ControllerTemplate {
    ...
    public Controller(Configuration conf, boolean isComments,boolean isSelenium) {
        ...
        sc = new SocketCommunication(ip,4700,6);
    }

    public static void main(String[] args) {

        Configuration conf = new Configuration(edu.ecnu.util.Config.crawlerRoot + "/conf/webcollector-dxyedu.xml");
        Controller controller = new Controller(conf,false,true);
        String suffix = conf.get("crawler.suffix");
        String publisher=conf.get("dxyedu.publisher");
        String sourceId=conf.get("dxyedu.sourceId");
        sc.sendMessage(sourceId +"\t" + publisher + "\t" + "start");
        controller.setEduCrawlerClassName("edu.ecnu.crawler.DxyEdu.DxyEduCrawler");
        controller.setNextPageCrawlerClassName("edu.ecnu.crawler.DxyEdu.DxyEduNextPageCrawler");
        controller.run();
...
//        BaseEduNextPageRamCrawler nextPageCrawler = new DxyEduNextPageCrawler();
//
...
//
//        /**********************************************/
//        BaseEduCrawler EduCrawler = new DxyEduCrawler(crawldb,true);
//
