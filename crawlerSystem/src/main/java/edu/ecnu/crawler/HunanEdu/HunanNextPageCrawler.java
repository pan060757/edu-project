 ...
 */
public class HunanNextPageCrawler extends BaseEduNextPageBreadthCrawler {
    ...
    public HunanNextPageCrawler(String crawlPath, boolean autoParse) {
    ...
    public Element getNextPageElement(Document doc,String url) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if(nextUrl.length()==url.length() && nextUrl.compareTo(url)>0)
            {
                return element;
            }
            else if(nextUrl.length()>url.length()){
                return element;
            }
            else if(nextUrl.contains("下5页")){
                return element;
            }
        }
        return null;
